...
project:
  ...
  expr-scanner.[h/c] :  The scanner interface and implementation files
  			for generating tokens from a simple
			expression contained in a file.
  expr-ast.[h/c]     :  The AST interface and implementation files. It
  			defines a data structure representation of the
			AST for simple expressions and prototypes for
			functions that operate over that AST
			structure.
  expr-parser.[h/c]  :  The parser interface and implementation. It
  			will use the scanner to read tokens and
			construct an AST.
  main.c	     :  The main entry point into the program.
